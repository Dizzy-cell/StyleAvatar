...
def train(args, loader, generator, discriminator, g_ema, g_optim, d_optim, device):
    ...
    for idx in pbar:
        ...
        if get_rank() == 0:
            pbar.set_description((f" l1: {l1_loss_val:.4f};"))

            if i % 1000 == 0:# and i != args.start_iter:
                with torch.no_grad():
                    sample = g_ema(cond_img).clamp(min=-1, max=1)
                    utils.save_image(
...
if __name__ == "__main__":
    ...
    generator = FaceUNet(args.input_size, args.output_size, args.style_dim, args.n_mlp, args.channel_multiplier).to(device)
    if args.use_concat:
        discriminator = Discriminator(args.output_size, 6).to(device)
    else:
        discriminator = Discriminator(args.output_size, 3).to(device)
    g_ema = FaceUNet(args.input_size, args.output_size, args.style_dim, args.n_mlp, args.channel_multiplier).to(device)
    g_ema.eval()
